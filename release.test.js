...
const PROD_CACHE = 'dreamboard-dreamboard-v20';
const PREVIEW_CACHE = 'dreamboard-dreamboard-v14-preview-v20';
...
test('1. version.txt: Stage 7B v15 identifier', () => {
    assert.ok(/Build: 2026-09-08-v20-board-appearance/.test(VERSION_TXT), 'Build = v15 Stage 7B');
...
test('2. version.txt: Expected cache описывает scoped runtime-name, не один глобальный', () => {
    ...
    assert.ok(/dreamboard-<scope>-v20/.test(VERSION_TXT), 'формула dreamboard-<scope>-v20');
    assert.ok(/dreamboard-dreamboard-v20/.test(VERSION_TXT), 'пример production имени');
    assert.ok(/dreamboard-dreamboard-v14-preview-v20/.test(VERSION_TXT), 'пример preview имени');
...
test('3. production scope: runtime cache name = dreamboard-dreamboard-v14', () => {
    ...
    assert.strictEqual(rt.precacheUrls.length, 30, '30 PRECACHE entries');
});
...
test('5. один source-файл вычисляет разные cache names (production != preview)', () => {
    ...
    assert.ok(prod.cacheName.endsWith('-v20') && prev.cacheName.endsWith('-v20'), 'оба версии v15');
});

// --- install ---------------------------------------------------------------

test('6. install (production) создаёт НОВЫЙ scoped cache со всеми 30 PRECACHE entries', async () => {
    ...
    assert.strictEqual(urls.size, 30, 'ровно 30 entries');
...
test('12. PRECACHE_URLS содержит все 30 entries, все файлы существуют на диске', () => {
    const m = SW_JS.match(/const PRECACHE_URLS = \[([\s\S]*?)\];/);
    assert.ok(m, 'PRECACHE_URLS найден');
    const urls = Array.from(m[1].matchAll(/'([^']+)'/g), x => x[1]);
    assert.strictEqual(urls.length, 30, 'ровно 30 entries');
