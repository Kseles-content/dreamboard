...
const PROD_CACHE = 'dreamboard-dreamboard-v32';
const PREVIEW_CACHE = 'dreamboard-dreamboard-v14-preview-v32';
...
test('1. version.txt: Stage 7B v15 identifier', () => {
    assert.ok(/Build: 2026-09-14-v32-dome-dream/.test(VERSION_TXT), 'Build = v15 Stage 7B');
...
test('2. version.txt: Expected cache описывает scoped runtime-name, не один глобальный', () => {
    ...
    assert.ok(/dreamboard-<scope>-v32/.test(VERSION_TXT), 'формула dreamboard-<scope>-v32');
    assert.ok(/dreamboard-dreamboard-v32/.test(VERSION_TXT), 'пример production имени');
    assert.ok(/dreamboard-dreamboard-v14-preview-v32/.test(VERSION_TXT), 'пример preview имени');
...
test('5. один source-файл вычисляет разные cache names (production != preview)', () => {
    ...
    assert.ok(prod.cacheName.endsWith('-v32') && prev.cacheName.endsWith('-v32'), 'оба версии v15');
});
...
test('14. SW: scope-изоляция реализована (SCOPE_NAME, SCOPE_OLD_RE, IS_PRODUCTION_SCOPE)', () => {
    assert.ok(/var CACHE_NAME = 'dreamboard-' \+ SCOPE_NAME \+ '-v32';/.test(SW_JS),
...
test('custom domain root scope installs and retains photo cache on upgrade', async () => {
    ...
    await runInstall(sw);
    await runActivate(sw);
    assert.ok(sw.opened.includes('dreamboard-root-v32'));
