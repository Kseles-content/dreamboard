...
const PROD_CACHE = 'dreamboard-dreamboard-v23';
const PREVIEW_CACHE = 'dreamboard-dreamboard-v14-preview-v23';
...
test('1. version.txt: Stage 7B v15 identifier', () => {
    assert.ok(/Build: 2026-09-10-v23-breath-rotation/.test(VERSION_TXT), 'Build = v15 Stage 7B');
...
test('2. version.txt: Expected cache описывает scoped runtime-name, не один глобальный', () => {
    ...
    assert.ok(/dreamboard-<scope>-v23/.test(VERSION_TXT), 'формула dreamboard-<scope>-v23');
    assert.ok(/dreamboard-dreamboard-v23/.test(VERSION_TXT), 'пример production имени');
    assert.ok(/dreamboard-dreamboard-v14-preview-v23/.test(VERSION_TXT), 'пример preview имени');
...
test('5. один source-файл вычисляет разные cache names (production != preview)', () => {
    ...
    assert.ok(prod.cacheName.endsWith('-v23') && prev.cacheName.endsWith('-v23'), 'оба версии v15');
});
...
test('14. SW: scope-изоляция реализована (SCOPE_NAME, SCOPE_OLD_RE, IS_PRODUCTION_SCOPE)', () => {
    assert.ok(/var CACHE_NAME = 'dreamboard-' \+ SCOPE_NAME \+ '-v23';/.test(SW_JS),
