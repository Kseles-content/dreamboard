...
const PROD_CACHE = 'dreamboard-dreamboard-v21';
const PREVIEW_CACHE = 'dreamboard-dreamboard-v14-preview-v21';
...
test('1. version.txt: Stage 7B v15 identifier', () => {
    assert.ok(/Build: 2026-09-08-v21-png-export/.test(VERSION_TXT), 'Build = v15 Stage 7B');
...
test('2. version.txt: Expected cache описывает scoped runtime-name, не один глобальный', () => {
    ...
    assert.ok(/dreamboard-<scope>-v21/.test(VERSION_TXT), 'формула dreamboard-<scope>-v21');
    assert.ok(/dreamboard-dreamboard-v21/.test(VERSION_TXT), 'пример production имени');
    assert.ok(/dreamboard-dreamboard-v14-preview-v21/.test(VERSION_TXT), 'пример preview имени');
...
test('3. production scope: runtime cache name = dreamboard-dreamboard-v14', () => {
    ...
    assert.strictEqual(rt.precacheUrls.length, 31, '31 PRECACHE entries');
});
...
test('5. один source-файл вычисляет разные cache names (production != preview)', () => {
    ...
    assert.ok(prod.cacheName.endsWith('-v21') && prev.cacheName.endsWith('-v21'), 'оба версии v15');
});

// --- install ---------------------------------------------------------------

test('6. install (production) создаёт НОВЫЙ scoped cache со всеми 31 PRECACHE entries', async () => {
    ...
    assert.strictEqual(urls.size, 31, 'ровно 31 entries');
...
test('12. PRECACHE_URLS содержит все 31 entries, все файлы существуют на диске', () => {
    const m = SW_JS.match(/const PRECACHE_URLS = \[([\s\S]*?)\];/);
    assert.ok(m, 'PRECACHE_URLS найден');
    const urls = Array.from(m[1].matchAll(/'([^']+)'/g), x => x[1]);
    assert.strictEqual(urls.length, 31, 'ровно 31 entries');
