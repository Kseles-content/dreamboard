...
const PROD_CACHE = 'dreamboard-dreamboard-v33';
const PREVIEW_CACHE = 'dreamboard-dreamboard-v14-preview-v33';
...
const EXPECTED_PRECACHE = [
    ...
    './assets/images/dream_career.png', './assets/images/dream_travel.png', './assets/images/dream_dome.png',
...
function loadSW(scopeUrl, existingCacheNames) {
    ...
    const sandbox = {
        ...
        Promise,
        URL,
        location: { origin: new URL(scopeUrl).origin, pathname: new URL(scopeUrl).pathname },
        caches: cachesMock,
...
test('1. version.txt: Stage 7B v15 identifier', () => {
    assert.ok(/Build: 2026-09-14-v33-existing-board-dome/.test(VERSION_TXT), 'Build = v15 Stage 7B');
...
test('2. version.txt: Expected cache описывает scoped runtime-name, не один глобальный', () => {
    ...
    assert.ok(/dreamboard-<scope>-v33/.test(VERSION_TXT), 'формула dreamboard-<scope>-v33');
    assert.ok(/dreamboard-dreamboard-v33/.test(VERSION_TXT), 'пример production имени');
    assert.ok(/dreamboard-dreamboard-v14-preview-v33/.test(VERSION_TXT), 'пример preview имени');
...
test('3. production scope: runtime cache name = dreamboard-dreamboard-v14', () => {
    ...
    assert.strictEqual(rt.precacheUrls.length, 36, '36 PRECACHE entries');
});
...
test('5. один source-файл вычисляет разные cache names (production != preview)', () => {
    ...
    assert.ok(prod.cacheName.endsWith('-v33') && prev.cacheName.endsWith('-v33'), 'оба версии v15');
});

// --- install ---------------------------------------------------------------

test('6. install (production) создаёт НОВЫЙ scoped cache со всеми 36 PRECACHE entries', async () => {
    ...
    assert.strictEqual(urls.size, 36, 'ровно 36 entries');
...
test('12. PRECACHE_URLS содержит все 36 entries, все файлы существуют на диске', () => {
    const m = SW_JS.match(/const PRECACHE_URLS = \[([\s\S]*?)\];/);
    assert.ok(m, 'PRECACHE_URLS найден');
    const urls = Array.from(m[1].matchAll(/'([^']+)'/g), x => x[1]);
    assert.strictEqual(urls.length, 36, 'ровно 36 entries');
...
test('14. SW: scope-изоляция реализована (SCOPE_NAME, SCOPE_OLD_RE, IS_PRODUCTION_SCOPE)', () => {
    assert.ok(/var CACHE_NAME = 'dreamboard-' \+ SCOPE_NAME \+ '-v33';/.test(SW_JS),
...
test('custom domain root scope installs and retains photo cache on upgrade', async () => {
    const sw = loadSW('https://dreamboard.kseles.ru/', ['dreamboard-root-v30', 'dreamboard-root-photos-v1']);
    await runInstall(sw);
    await runActivate(sw);
    assert.ok(sw.opened.includes('dreamboard-root-v33'));
    assert.ok(sw.deleted.includes('dreamboard-root-v30'));
    assert.ok(!sw.deleted.includes('dreamboard-root-photos-v1'));
});
