...
[README проекта](../README.md) · Актуально на 2026-09-14.
...
Кнопки лендинга ведут в приложение по относительному адресу, сохраняя preview или production. Баннер целиком является ссылкой на `https://dreamboard.kseles.ru/welcome/?utm_source=personal`. В PNG ссылки нет: при размещении назначьте переход на лендинг. Допустимые метки статистики: `?utm_source=telegram`, `personal` или `pilot`.

## Оформление и поведение

PNG: **1080 × 1350 пикселей**. HTML-баннер адаптивен, с пропорцией 4:5. Использованы существующие `assets/images/dream_dome.png` и `dream_career.png`, Arial и Georgia, цвета `#f5f3ec`, `#26382e`, `#a6482e`, `#e2e8d9`.
...
В v33 используется предоставленный владельцем исходник `Dome House.png` (светлый дом у моря); путь в приложении остаётся `assets/images/dream_dome.png`. Изображение заменяется вместе с обновлением офлайн-кэша.

## Согласованные тексты

Название нового примера: «Построить свой купольный дом».

Описание: «Необычное, светлое и уютное пространство для всей семьи, в гармонии с природой».

При замене изображений сохранять основной текст лендинга и метаданные описаний ссылок. Менять эти тексты только по согласованию с владельцем проекта. Нынешние подписи приложения и лендинга сохранены по его решению. На лендинге и баннере два сюжета: IT-стартап и купольный дом. Прежние примеры приложения не удалять.
