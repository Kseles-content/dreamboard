...
test('34. backup.js присутствует в PRECACHE, CACHE_NAME scoped (dreamboard-<scope>-v22)', () => {
    const sw = fs.readFileSync(path.join(__dirname, 'service-worker.js'), 'utf8');
    assert.ok(sw.includes("'./backup.js'"), 'backup.js в PRECACHE_URLS');
    assert.ok(sw.includes("var CACHE_NAME = 'dreamboard-' + SCOPE_NAME + '-v22';"), 'CACHE_NAME scoped по scope');
});
