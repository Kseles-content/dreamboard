...
- **Mitigations:**
  ...
  - Storage path **`{user_id}/{board_id}/{filename}`** with policies enforcing ALL of: segment 1 = `auth.uid()`, segment 2 = valid UUID **and an existing `sync_documents` board owned by the caller** (foreign/nonexistent boards rejected), **exactly two folder segments** (`cardinality(foldername) = 2`, extra directories rejected) and **non-empty file name** (`storage.filename(name)`); `WITH CHECK` on insert/update; signed URLs only (short TTL).
  - **Signed URL semantics (bearer link):** a signed URL issued by the owner is a **temporary bearer link** — anyone holding it can access the object until expiry (this is the intended contract, not a leak; no 403 is expected on opening an already-issued URL). Mitigations: issue URLs only per explicit user action, keep TTL short, and never expose a URL in logs/chats. Creating a signed URL for **someone else's** object is denied by the select policy (verified in the Storage gate: B cannot create a signed URL for A's object).
