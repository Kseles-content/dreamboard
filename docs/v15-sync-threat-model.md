...
- **Mitigations:**
  ...
  - All writes through `sync_push_document`/`sync_snapshot` (SECURITY INVOKER, `search_path` pinned, RLS applies); no direct client table access.
  - Storage path convention `dream-images/{user_id}/...` enforced by `WITH CHECK` on first path segment; signed URLs only (short TTL).
  - Cross-user tests T1–T9 in the SQL file (A/B/anon isolation) are part of the 7A gate; re-run in sandbox before 7C.
...
- **7C:** RLS cross-user suite re-run (T1–T9); CAS RPC fuzz (bad types, huge payloads, missing args); offline/reconnect; two-device conflict; token revocation test.
